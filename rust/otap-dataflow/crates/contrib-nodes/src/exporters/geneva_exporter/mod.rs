...
use otap_df_engine::capability::{
    BearerTokenProvider as BearerTokenProviderCap, VendorBundle as VendorBundleCap,
};
...
impl GenevaExporter {
    ...
    pub fn from_config(
        ...
        let auth_method = match &config.auth {
            ...
            // Unused on the agent-fed path (the token comes from the capability
            // via `with_agent_fed_source`); only satisfies the shared config struct.
