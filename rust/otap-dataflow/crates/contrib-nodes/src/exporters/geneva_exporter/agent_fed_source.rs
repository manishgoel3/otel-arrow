...
use otap_df_engine::shared::capability::{BearerTokenProvider, VendorBundle};
...
impl AgentFedCredentialSource for AgentFedGenevaSource {
    fn current(&self) -> AgentFedCredentialFuture<'_> {
        Box::pin(async move {
            // Token and routing are read under separate locks: a reseed between
            // them can transiently mismatch, self-healing via 401 -> refresh -> retry.
            ...
            let attributes = self.vendor.lock().await.attributes();
    ...
    fn on_unauthorized(&self) -> AgentFedRefreshFuture<'_> {
        // Forward the 401/403 signal to the token provider.
        Box::pin(async move {
            self.bearer.lock().await.request_refresh().await;
        })
    }
}
...
mod tests {
    ...
    use otap_df_engine::capability::{BearerToken, CapabilityError, RefreshFuture, TokenStream};
    ...
    async fn returns_credential_when_token_and_routing_present() {
        let s = source("tok", false, full_attrs(), Arc::new(AtomicUsize::new(0)));
    ...
    async fn awaits_pending_provider_future() {
        ...
        let s = source("tok", true, full_attrs(), Arc::new(AtomicUsize::new(0)));
        assert!(s.current().await.is_some());
    }

    #[tokio::test]
    async fn fails_closed_on_empty_token() {
        let s = source("", false, full_attrs(), Arc::new(AtomicUsize::new(0)));
    ...
    async fn fails_closed_on_missing_endpoint() {
        ...
        let s = source("tok", false, attrs, Arc::new(AtomicUsize::new(0)));
        assert!(s.current().await.is_none());
    }
    ...
    #[tokio::test]
    async fn on_unauthorized_signals_provider_refresh() {
        let refreshes = Arc::new(AtomicUsize::new(0));
        let s = source("tok", false, full_attrs(), refreshes.clone());
        s.on_unauthorized().await;
